...
module Tsang
  class Parser
    ...
    def parse_select
      ...
      # Parse JOINs
      joins = []
      while [:JOIN, :LEFT, :RIGHT, :INNER, :OUTER].include?(current_token.type)
        joins << parse_join
      end
      ...
      loop do
        if current_token.type == :ASTERISK
          ...
          if current_token.type == :AS
            ...
            unless [:EOF, :COMMA, :FROM, :WHERE, :GROUP, :HAVING, :ORDER, :LIMIT, :SEMICOLON].include?(next_type)
              alias_name = consume(:IDENTIFIER).value
            end
          end
    ...
    def parse_table_reference
      ...
      unless name_token.type == :IDENTIFIER
        raise "Expected identifier but got #{name_token.type}"
      end
